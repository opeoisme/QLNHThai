    ...
    public partial class frmUnit : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void LoadFormUnit()
        {
            var catalogData = dbContext.Units
                             .Where(ct => ct.condition_Unit == "Sử dụng")
            ...
            if (focusedRowHandle >= 0)
            {
                ...
                if (selectedUnit != null)
                {
                    if (int.TryParse(selectedUnit.id_Unit.ToString(), out int unitId))
                    {
                        ...
                        {
                            unitToUpdate.condition_Unit = "Ngừng sử dụng";
                            dbContext.Entry(unitToUpdate).State = EntityState.Modified;
                            dbContext.SaveChanges();
                            LoadFormUnit();
                            XtraMessageBox.Show("Đơn vị ngừng sử dụng !", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                }
            }
        ...
        {
            if (int.TryParse(txbIdUnit.Text, out int unitId))
            {
                Unit unitToUpdate = dbContext.Units.FirstOrDefault(ct => ct.id_Unit == unitId);

                if (unitToUpdate != null)
                {
                    UpdateUnitProperties(unitToUpdate);
        ...
        {
            unit.name_Unit = txbNameUnit.Text;
        ...
        {
            frmAddUnit addUnitForm = new frmAddUnit();
            DialogResult result = addUnitForm.ShowDialog();
